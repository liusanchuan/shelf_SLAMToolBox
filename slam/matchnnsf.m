...
%
%-=-=匹配全局地图中和机器人局部地图中最邻近的点，作为两个测量中的相同特征来对待
%
...
if (nG > 0) & (nL > 0),
  ...
  for i = 1:nL,%局部地图中的特征点数local
    Cli = Cl(i,i).C;
    for j = 1:nG,   %全局地图中的特征点数
      Cgj = get(Xg{j},'c');%xg:特征的数组，c是特征的协方差
      nu = calcinnovation(Xl{i},Xg{j});     %返回两个状态向量的差 nu = A.x-B.x;
      S  = Cgj + Cli;
      if det(S) < 0, disp('--> matchnnsf: S negative definite'); end;
      %D（i，j）为每个局部特征点i与全局特征点i之间的马氏距离d，均值nu，方差R
      D(i,j).d  = mahalanobis(nu,S);% 计算马氏距离 d = v'*inv(S)*v;
      D(i,j).nu = nu;
      D(i,j).R  = Cli;
    end;
  end;
  ...
  for i = 1:nassoc,
    if associations(i).ig > 0,
      ...
      if (nl == 1) & (ng == 1),
        associations(i).accept = 1;%表示这一匹配是一对一
      else
        associations(i).accept = 2;%表示这一匹配是一对多
      end;
    else
